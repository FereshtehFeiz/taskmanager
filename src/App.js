...
class App extends React.Component {
  ...
  filterTasks = (filter, project) => {
    switch (filter) {
      ...
      case "filter-today":
        ...
        break;
      case "filter-week":
        API.getWeeklyTasks().then((tasks) =>
          this.setState({ tasks: tasks, filter: "Next Week" })
        );
        break;
      case "filter-private":
        API.getPrivateTasks().then((tasks) =>
          this.setState({ tasks: tasks, filter: "Private" })
        );
        break;
      case "filter-shared":
        API.getSharedTasks().then((tasks) =>
          this.setState({ tasks: tasks, filter: "Shared" })
        );
        break;
      case "filter-project":
        API.getByProject(project).then((tasks) =>
  ...
  addOrEditTask = (task) => {
    this.setState((state) => {
      ...
      if (!task.id) task.id = state.tasks.slice(-1)[0].id + 1;
